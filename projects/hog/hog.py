...
def free_bacon(score):
    ...
    k=100
    i=0
    while i<=score:     # for i in range(score+1):使用for loop另一种方法
        p=pow(10,k-i)
        digits=pi//p
        pi=pi%p
        i=i+1
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        if other(who)==1:
            player_num_rolls0=strategy0(score0,score1)
            this_turn_score0=take_turn(player_num_rolls0,score1,dice)
            ...
            player_num_rolls1=strategy1(score1,score0)
            this_turn_score1=take_turn(player_num_rolls1,score0,dice)
...
def make_averaged(original_function, trials_count=1000):
    ...
    #another solution#
    # def average(*args):
        #average=[original_function(*args) for i in range(trails_count)]
        #return sum(average)/trials_count
    #return average
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    max_average=0
    for i in range(1,11):
        get_averaged=make_averaged(roll_dice,trials_count=1000)
        max_average_rolls=get_averaged(i,dice)
        if max_average_rolls>max_average:
            max_num_rolls=i
            max_average=max_average_rolls
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    bacon_entra_strategy=free_bacon(opponent_score)
    if extra_turn(score+bacon_entra_strategy,opponent_score):
        return 0
    else:
