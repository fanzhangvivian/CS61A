...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    if k < len([paragraph for paragraph in paragraphs if select(paragraph)]):
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def helper(x):
        x_list=split(lower(remove_punctuation(x)))
        for i in topic:
            if i in x_list:
                return True
        return False
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for i in valid_words:
        if diff_function(user_word,i,limit)==min_diff:
            return i
    # END PROBLEM 5
...
def shifty_shifts(start, goal, limit):
    ...
    if start==goal:
        return 0
    if limit<1 or start=='' or goal=='':
        #如果其中一个为空字符串，则为剩余的字符串最大值
